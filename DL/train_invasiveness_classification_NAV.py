...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='invasiveness_NAV')
    ...
    # Ndict = {k: v for k, v in torch.load('DL/models/M12_N_pretrained.pt').items() if
    #          'fc' not in k}
    # model.modelN.load_state_dict(Ndict, strict=False)
    # Adict = {k: v for k, v in torch.load('DL/models/M12_A_pretrained.pt').items() if
    #          'fc' not in k}
    # model.modelA.load_state_dict(Adict, strict=False)
    # Vdict = {k: v for k, v in torch.load('DL/models/M12_V_pretrained.pt').items() if
    #          'fc' not in k}
    ...
    train_dataset = HybridSlice(csv_path='csv/demo_split.csv',
                                fold='train', binary='12', augmentation=True, normal=True)
    ...
    val_dataset = HybridSlice(csv_path='csv/demo_split.csv',
                              fold='val', binary='12', augmentation=False, normal=True)
