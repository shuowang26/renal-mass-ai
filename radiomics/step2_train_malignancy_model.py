...
# This script trains radiomics model for malignancy prediction.
...
y_train = np.array(y_train > 0).astype(int)  # binary classification label for malignancy

# get validation set
val_feat = radiomics_feat[df['split'] == 'val']
X_val = val_feat.values[:, 2:].astype(np.float32)
y_val = val_feat.values[:, 1].astype(int)
y_val = np.array(y_val > 0).astype(int)
...
joblib.dump(selected_features, 'radiomics/trained_models/model01_selected_features.pkl')
joblib.dump(scaler, 'radiomics/trained_models/model01_scaler.pkl')
joblib.dump(cv_results, 'radiomics/trained_models/model01_cv_results.pkl')
joblib.dump(selected_index, 'radiomics/trained_models/model01_selected_index.pkl')